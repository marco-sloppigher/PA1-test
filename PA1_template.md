...
The next chunk, which does not evaluate, can be used as an alternative to the previous one, in case we want to plot the difference from the mean instead of the value. The shape of the plot will be the same, only the scale on the y-axis will change.
...
# It can be used later to produce an alternative plot.
